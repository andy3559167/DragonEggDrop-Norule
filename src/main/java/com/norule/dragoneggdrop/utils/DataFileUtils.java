...
public final class DataFileUtils {
    ...
    public static void reloadInMemoryData(@NotNull DragonEggDrop plugin, boolean log) {
        ...
        for (File file : plugin.getParticleDirectory().listFiles((file, name) -> {
            String baseName = JsonUtils.getBaseName(name);
            return JsonUtils.hasSupportedFileExtension(name) && !baseName.equals("possible_conditions");
        })) {
