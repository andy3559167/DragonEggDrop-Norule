package com.norule.dragoneggdrop.dragon;
...
public final class DragonTemplate implements Registerable {
    ...
    public void applyToBattle(@NotNull EnderDragon dragon, @NotNull DragonBattle battle) {
        ...
        dragon.getPersistentDataContainer().set(DragonEggDrop.getInstance().getDragonTemplateKey(), PersistentDataType.STRING, this.id);
        ...
        Attribute maxHealthAttribute = AttributeUtils.parseAttribute("minecraft:generic.max_health");
        if (maxHealthAttribute != null && attributes.containsKey(maxHealthAttribute)) {
            dragon.setHealth(attributes.get(maxHealthAttribute));
        }
    }
    ...
    public static DragonTemplate fromFile(@NotNull File file) {
        ...
        if (attributeSection != null) {
            for (String attributeKey : attributeSection.getValues(false).keySet()) {
                Attribute attribute = AttributeUtils.parseAttribute(attributeKey);
                if (attribute == null) {
