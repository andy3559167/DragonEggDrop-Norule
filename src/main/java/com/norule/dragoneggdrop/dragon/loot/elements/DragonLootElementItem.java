package com.norule.dragoneggdrop.dragon.loot.elements;
...
import com.norule.dragoneggdrop.utils.AttributeUtils;
import com.norule.dragoneggdrop.utils.IntegerRange;
...
public final class DragonLootElementItem implements IDragonLootElement {
    ...
    public static DragonLootElementItem fromJson(@NotNull JsonObject root) throws JsonParseException {
        ...
        if (root.has("attribute_modifiers")) {
            ...
            for (Entry<String, JsonElement> modifierEntry : modifiersRoot.entrySet()) {
                Attribute attribute = AttributeUtils.parseAttribute(modifierEntry.getKey());
                if (attribute == null) {
        ...
        if (meta instanceof BannerMeta) {
            ...
            if (root.has("patterns")) {
                ...
                for (JsonElement patternElement : patternsElement.getAsJsonArray()) {
                    ...
                    PatternType pattern = parsePatternType(patternRoot.get("pattern").getAsString());
                    if (pattern == null) {
                        throw new JsonParseException("Unexpected value for \"pattern\". Given \"" + patternRoot.get("pattern").getAsString() + "\", expected https://hub.spigotmc.org/javadocs/spigot/org/bukkit/block/banner/PatternType.html");
                    }
        ...
        if (meta instanceof PotionMeta) {
            ...
            PotionType basePotionType = root.has("base") ? parsePotionType(root.get("base").getAsString(), PotionType.WATER) : PotionType.WATER;
        ...
        if (meta instanceof SkullMeta) {
            ...
            if (root.has("owner")) {
                String ownerRaw = root.get("owner").getAsString().trim();
                if (!ownerRaw.isEmpty()) {
                    UUID ownerUuid = parseOptionalUuid(ownerRaw);
                    if (ownerUuid != null) {
                        metaSpecific.setOwnerProfile(Bukkit.createPlayerProfile(ownerUuid));
                    } else {
                        try {
                            metaSpecific.setOwnerProfile(Bukkit.createPlayerProfile(ownerRaw));
                        } catch (IllegalArgumentException ignored) {
                            metaSpecific.setOwner(ownerRaw);
                        }
                    }
                }
            }
    ...
    @Nullable
    private static UUID parseOptionalUuid(@NotNull String value) {
        try {
    ...
    @Nullable
    private static PatternType parsePatternType(@NotNull String value) {
        PatternType byIdentifier = PatternType.getByIdentifier(value.toLowerCase(Locale.ROOT));
        if (byIdentifier != null) {
            return byIdentifier;
        }

        try {
            return PatternType.valueOf(value.toUpperCase(Locale.ROOT));
        } catch (IllegalArgumentException ignored) {
            NamespacedKey key = NamespacedKeyUtil.fromString(value, null);
            return key != null ? Registry.BANNER_PATTERN.get(key) : null;
        }
    }

    @NotNull
    private static PotionType parsePotionType(@NotNull String value, @NotNull PotionType fallback) {
        PotionType byEnumName = Enums.getIfPresent(PotionType.class, value.toUpperCase(Locale.ROOT)).orNull();
        if (byEnumName != null) {
            return byEnumName;
        }
        ...
        PotionType byRegistry = Registry.POTION.get(key);
        return byRegistry != null ? byRegistry : fallback;
    }
