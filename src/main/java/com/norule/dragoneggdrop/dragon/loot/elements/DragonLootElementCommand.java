...
public class DragonLootElementCommand implements IDragonLootElement {
    ...
    public void generate(@Nullable DragonBattle battle, @NotNull DragonTemplate template, @Nullable Player killer, @NotNull Random random, @Nullable Chest chest) {
        ...
        String parsedCommand = DragonEggDropPlaceholders.inject(killer, command.replace("%dragon%", template.getName()));
        Runnable dispatcher = () -> Bukkit.dispatchCommand(Bukkit.getConsoleSender(), parsedCommand);

        DragonEggDrop plugin = DragonEggDrop.getInstance();
        if (plugin.getTaskScheduler().isFolia()) {
            // Command dispatch must run on Folia's global scheduler.
            plugin.getTaskScheduler().runLater(null, 0L, dispatcher);
            return;
        }
