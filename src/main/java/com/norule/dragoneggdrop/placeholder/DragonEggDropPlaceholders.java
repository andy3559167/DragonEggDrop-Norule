package com.norule.dragoneggdrop.placeholder;
...
import com.norule.dragoneggdrop.dragon.DamageHistory.DamageEntry;
...
public final class DragonEggDropPlaceholders {
    ...
    static String replacePlaceholder(@Nullable OfflinePlayer player, @NotNull String placeholder) {
        if (placeholder.equalsIgnoreCase("dragon")) { // %dragoneggdrop_dragon%
            ...
            int offset = (matcher.group(1) != null) ? parseInteger(matcher.group(1), 0) - 1 : 0;
            if (offset < 0) {
                return null;
            }
    ...
    private static int parseInteger(@NotNull String value, int fallback) {
        try {
